...
def create_crew_record():
    ...
    try:
        ...
        crew_service.create_crew_record(proj_id, json.loads(lines))
...
def get_crew_record():
    ...
    try:
        ...
        data = crew_service.get_crew_records(proj_id, page, filters)
        return jsonify(status='ok', content=data)
    except Exception, ex:
        traceback.print_exc()
        return jsonify(status='error', msg=ex.message)
...
def delete_crew_record():
    ...
    try:
        ...
        data = crew_service.delete_crew_records(proj_id, crew_id)
        return jsonify(status='ok', content=data)
    except Exception, ex:
        return jsonify(status='error', msg=ex.message)